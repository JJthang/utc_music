...
import type { AlbumFeatured } from "@/types/stats.type";
import { getFeaturedAlbums } from "@/services/Apis/stats.service";
...
const FeaturedAlbum = () => {
  const [featuredAlbum, setFeaturedAlbum] = useState<AlbumFeatured[]>([]);
  const [isFetching, setIsFetching] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchFeaturedAlbum = async () => {
      try {
        const response = await getFeaturedAlbums();
        setFeaturedAlbum(response.data);
      } catch (error) {
        console.error("Error fetching:", error);
        setError(
          "Không thể tải danh sách album nổi bật, vui lòng thử lại sau!"
        );
      } finally {
        setIsFetching(false);
      }
  ...
  return (
    <Fragment>
      {featuredAlbum.length === 0 ? null : (
        <section className="w-full p-2 sm:p-4">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">Top Album nổi bật</h2>
            <button
              type="button"
          ...
          <HorizontalScrollWrapper>
            {featuredAlbum?.map((album, index) => (
              <FeaturedAlbumCard key={index} album={album} />
            ))}
          </HorizontalScrollWrapper>
        </section>
      )}
